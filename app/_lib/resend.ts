...
export async function sendPrizeEmail(
  ...
  const subject =
    prize.type === "discount"
      ? `Your Figur Prize: ${prize.headline}`
      : `You earned ${prize.pointsAwarded} Figur Points`;
  ...
  const html =
    prize.type === "discount" && code
      ? discountEmailHtml(firstName, prize, code, cartUrl)
      : pointsEmailHtml(firstName, prize);
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr><td align="center" style="padding:48px 16px;">
          <table width="520" cellpadding="0" cellspacing="0" style="background:#660033;padding:40px 48px;">
            ...
            <tr><td style="border-top:1px solid rgba(197,162,88,0.3);padding-top:32px;">
              <p style="margin:0 0 16px;color:#C5A258;font-size:12px;letter-spacing:3px;text-transform:uppercase;">Dear ${firstName},</p>
              <p style="margin:0 0 24px;color:#FFFBF4;font-size:16px;line-height:1.6;">Congratulations — you hold ${prize.pointsAwarded} Figùr Loyalty Points.</p>
              <h2 style="margin:0 0 8px;color:#C5A258;font-size:48px;font-weight:400;text-align:center;">${prize.pointsAwarded}</h2>
              <p style="margin:0 0 24px;color:rgba(255,251,244,0.6);font-size:14px;text-align:center;">Figùr Loyalty Points</p>
              <p style="margin:0;color:rgba(255,251,244,0.5);font-size:12px;line-height:1.7;">These points are banked to your account and will be redeemable when the Figùr loyalty program launches. You will receive a notification when they are ready to use.</p>
