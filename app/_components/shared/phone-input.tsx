...
export function PhoneInput({ value, onChange, onKeyDown, error }: PhoneInputProps) {
      ...
      <Popover.Root open={open} onOpenChange={(o) => { if (o) handleOpen(); else setOpen(false); }}>
        ...
        <Popover.Portal>
          <Popover.Content
            ...
            avoidCollisions
            collisionPadding={16}
            className="z-50 w-72 bg-white rounded-2xl shadow-xl border border-plum/8 overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95"
          >
            <div className="p-2 border-b border-plum/6">
              <input
                ref={searchRef}
                type="text"
                value={search}
                ...
                className="w-full bg-plum/4 rounded-xl px-3 py-2 text-sm font-body text-plum-deep placeholder:text-plum/30 outline-none focus:bg-plum/6 transition-colors"
              />
            </div>
            <ul className="max-h-60 overflow-y-auto py-1.5">
              {filtered.length === 0 ? (
                <li className="px-4 py-3 text-sm font-body text-plum/40 text-center">No results</li>
                ...
                <li key={country.code}>
                  <button
                    ...
                    className={`
                      ...
                      font-body text-sm text-plum-deep hover:bg-butter/15 transition-colors
                      ${selected.code === country.code ? "bg-butter/10 font-medium" : ""}
                    `}
                  >
                    <span className="text-lg">{country.flag}</span>
                    <span className="flex-1">{country.name}</span>
                    <span className="text-plum/40 text-xs tabular-nums">{country.dial}</span>
                  </button>
                </li>
              ))}
            </ul>
